...
liquidSensor::status liquidSensor::getLevel(uint8_t *getlevel) {
	...
	if (LiquidMeasSemaphore.semaphoreTake(1000)
		...
		for (int i = 0; i < 800; i++) {
			CheckError1(HAL_ADC_Start(hadc_sensor), liquidSensor_ERR, HAL_OK);
			CheckError1(HAL_ADC_PollForConversion(hadc_sensor, 10000),
...
void liquidSensor::refuelingDetection(uint32_t curEpochTime,
	...
	if (LiquidMeasSemaphore.semaphoreTake(1000) == System_Rtos::freertos_semaphore::semaphore_recived) {
		...
		if (!refueling_active) {
			...
			if (volume_change > 3 && ((refueling_detect_start_time - refueling_detect_prev_time) >= refuel_detect_stabilize_time)) {
				refueling_active = 1;
